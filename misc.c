...
// definition of the push and pop functions

void push(struct sebuf *sem, int num)

		{
		            sem->procw[sem->tail]=num;
		            sem->tail = (sem->tail+1)%NR_PROCS; // for circular queue to happen
		}

int pop(struct sebuf *sem)

		{
		            int ret = sem->procw[sem->head];
		            sem->procw[sem->head]=0;
		            sem->head = (sem->head+1)%NR_PROCS; // for circular queue to happen 
		            return ret;
		}
...
register struct mproc *rmp, *rmp2,*rmp3; 
int k,m,s;
int child_pid=m_in.m1_i1;
for ( rmp = & mproc[0]; rmp < & mproc[NR_PROCS]; rmp++ )
{

if(rmp->mp_pid==child_pid) 
...
rmp3 = & mproc[s];
m = rmp3->mp_pid;
printf(" Promotion done successfully. I am child with PID %d . My parent's ID is %d ",rmp->mp_pid, m);
break;
}
}  
return 0;
...
// catching the descriptor (with which the semaphore has to be created)
  
  	      if (mp->semarray[desc] != NULL)  
  	
// if the element of the semarray at "desc" is NOT NULL
  		  
  		  {
	  			printf("Descriptor in use. Please try again. \n");
	  			return -1;
	  	   }
	  	   
          if ( mp->semarray[desc] == NULL) 
    
// if the element of the semarray at desc is NULL
	  	   
	  	   {
			  mp->semarray[desc] = (struct sebuf *)malloc(sizeof(struct sebuf));
			  
			  // memory allocation for the structure of the semaphore
			  
			  mp->semarray[desc]->desc = desc;
			  mp->semarray[desc]->state = 1;
			  mp->semarray[desc]->ref_count = 1;
			  
			  // setting values to the elements of the structure.
			  
   			  printf("Semaphore created = %d  ref_count= %d.\n",mp->semarray[desc]->desc, mp->semarray[desc]->ref_count);
   			  return 1;
		    }
...
// catching the descriptor on which UP has to be done. 
			
			if(mp->semarray[desc] != NULL)
			{

				if(mp->semarray[desc]->state == 1)
				{
				   printf("The process is already up \n");
				   return -1;
				}
			else if( mp->semarray[desc]->state ==0 &&  mp->semarray[desc]->procq ==0 )

				{
				   mp->semarray[desc]->state = 1;
				   printf("The Semaphore has been released. Now you should be able to use the semaphore! \n");
				   return 1;
				}

			else
				{
					int procindex = pop(mp->semarray[desc]); 
					
					// the first process waiting in the queue is taken out. 
					
					setreply(procindex, OK); 
					
					// a signal is sent to that process so that it awakes
					
					mp->semarray[desc]->processq--; 
					
					// the number of processes waiting in the queue is decreased by 1
...
// catching the descriptor on which down has to be done
		...
		if (mp->semarray[desc]!=NULL)
			{
			   if(mp->semarray[desc]->state == 1)
			   
				{
				 ...
				 mp->semarray[desc]->state=0;
				 printf("Semaphore was free to use before. But now I entered into my critical region.\n");
				 return 1; 
			   ...
			   push(mp->semarray[desc],who_p); 
			   
			   // A down cannot be done because some other process is using the semaphore
			   // So that process which does a down is put into the queue.
			   
			   mp->semarray[desc]->processq++;
			   
			   // The number of processes in the queue is increased by 1.
			   
			   printf("The semaphore is not free to use. You have been put into the waiting list! \n");
			   return SUSPEND;
...
// catching the descriptor on which CLOSE has to be done. 

printf("The descriptor value is %d \n",desc);

 	    if (mp->semarray[desc]->ref_count > 0)
   			{
			   mp->semarray[desc]->ref_count--;
			   
			   // The ref_count is decreased by 1 because a CLOSE is done by just one process
			   // Other processes might be still waiting on that semaphore.
			   
			   printf("ref_count of the semaphore now is = %d \n",mp->semarray[desc]->ref_count);
			   mp->semarray[desc]= NULL;
			   return -1;
   			}
		else if (mp->semarray[desc]->ref_count == 0)
   			{
		  	   free(mp->semarray[desc]); 
		  	   
		  	   // free the memory if the ref_count is zero
		  	   // This means no process is waiting or intending to use that semaphore.
