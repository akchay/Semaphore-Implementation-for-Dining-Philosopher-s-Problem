...
/* The process pointed to by 'mp' has forked.  Create a child process. */
  ...
  *rmc = *rmp;			/* copy parent's process slot to child's */
   ...
   /* 
   rmc is pointer to child process
   rmp is pointer to parent process 
   */

  {
 
     rmc->semarray[k]=rmp->semarray[k]; // explicitly copying the semarray (of pointers) to child. 
    
	  	  if(rmp->semarray[k]!= NULL) // if semaphore (desc) has been created by parent then 
	  	  	  {
	  		  	  rmp->semarray[k]->ref_count++;
	  		  	  printf("The ref count is %d", rmp->semarray[k]->ref_count);
	  		  	  
	  		  	  // increasing the ref_count by 1 when child inherits a pointer to the semaphore which is not NULL
	  	 	  }
 ...
 */
  ...
  for (int i=0; i<12;i++)
    {
  	   if (mp->semarray[i]!=NULL)

  	    {

	  		   if(mp->semarray[i]->ref_count > 0)
	  		   {
	  		   mp->semarray[i]->ref_count--; 
	  		   
	  		   // decreasing the ref_count by 1 
	  		   // there might be some other process waiting on that semaphore.
	  		   
	  		   mp->semarray[i] = NULL;
	  		   }
	
	  		   else if (mp->semarray[i]->ref_count == 0)
	  		   {
	  		   
	  		   free(mp->semarray[i]);
	  		   
	  		   // free the pointer if no process is waiting on a semaphore.
